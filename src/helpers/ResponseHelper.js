/*
    Borrowed from a senior colleague :-)
 */
...
const errorResponse = R.curry((req, res, err) => {
    if (err.code === 11000) {
        return res.status(400).send({ errors: [{ level: 'ERROR', message: 'email address is already in use'}] });
    }
    return res.status(500).send({ errors: [{ level: 'ERROR', message: err }] })
});
